...
module.exports = {
		...
		if (true) {	// 未完成
			...
			if (char.age == -1 || char.age_t == -1) {
				...
				if (char.age_t == -1) {
					...
					age = this.setStatusByTalent(talent_skill_data, age, "age_inside");	// 依角色心理、生理狀態(age_inside)調整角色外表年齡
					age = age * lifespan / race_data.getData(0, "lifespan");	// 計算種族加權(以外表年齡和種族壽命計算出真實年齡)
					age = this.setStatusByTalent(talent_skill_data, age, "age_exp");	// 以角色經歷(age_exp)調整角色真實年齡
				...
				if (char.age == -1) {
					...
					age = this.setStatusByTalent(talent_skill_data, age, "age_outside");	// 以角色外貌(age_outside)調整角色外表年齡
			...
			if (char.height == -1 || char.weight == -1 || char.body_b == -1 || char.body_w == -1 || char.body_h == -1) {
				let height, weight, body_b, body_w, body_h;
				...
				// 依據屬性調整身高體重
				height = this.setStatusByTalent(talent_skill_data, height, "height");
				weight = this.setStatusByTalent(talent_skill_data, weight, "weight");
				...
				// 依據屬性調整三圍
				body_b = this.setStatusByTalent(talent_skill_data, body_b, "body_b");
				body_w = this.setStatusByTalent(talent_skill_data, body_w, "body_w");
				body_h = this.setStatusByTalent(talent_skill_data, body_h, "body_h");
	...
	// 依據技能調整屬性
	setStatusByTalent: function (talent_skill_data, value, item) {
		for (let skill_id in talent_skill_data) {
			let effect = talent_skill_data[skill_id];

			if (adjust = effect[item]) {
	...
	tranCharData: function (char, item, showHidden) {
		...
		if (item == "gender") {	// 性別
			...
			// 非男性 計算罩杯
			if (char.gender != "3") {
				let body_b_u = char.height * 0.43;
				let bust_value = Math.round((char.body_b - body_b_u) / 2.5);
				str += "("
				if (bust_value == 0) str += "-";
