...
const race_util = require('./util_race.js');
const skill_util = require('./util_skill.js');
...
module.exports = {
	...
	newChar: function (char_no, data) {
		...
		if (true) {
			...
			char.race = char.race.filter(function (race_id) {	// 移除不存在之種族
				return race_util.contains(race_id);
			});
			if (char.race.length == 0) {	// 無種族時從隨機種族列表中挑選
				let base_race_list = race_util.getBaseData();
			...
			// 移除無繁衍能力之種族的混血種族
			if (race_util.getData(char.race[0], "gender")[1] == 100) {
				char.race.length = 1;
			} else {
				char.race = char.race.filter(function (race_id) {
					return race_util.getData(race_id, "gender")[1] != 100;
				});
			}
			...
			if (char.race.length < 2) {
				...
				if (rate == 0 || rate == 100) {
					...
					let race_r = race_util.getData(char.race[0], "rate");	// 主要種族權重
					let race_mr = race_util.getData(char.race[1], "rate");	// 混血種族權重
			...
			// 決定角色性別
			if (char.gender === "" || race_util.getData(char.race[0])["gender"][char.gender] == 0) {	// 未指定性別 / 該種族無指定指性別
				if (char.race.length > 1) {
					char.gender = util.getRandomWeightedValue(race_util.getData(char.race[0], "gender"), race_util.getData(char.race[1], "gender"), char.race_r);
				} else {
					char.gender = util.getRandomWeightedValue(race_util.getData(char.race[0], "gender"));
				}
			}
			...
			if (char.random_skill) {
				...
				let base_skill_list = skill_util.getBaseData();

				for (let skill_group of base_skill_list) {
					if (skill_util.chkEnableSkill(char, null, skill_group.flag)) {	// 符合起始技能獲取條件
						let skill_no = util.getRandomListValue(skill_group.skill_rate);

						if (skill_no != "") {
							if (skill_util.chkEnableSkill(char, skill_no)) char.skill.push(skill_no);	// 符合技能獲取條件
						}
					}
				}
			...
			for (let skill_no of skill_list) char.skill[skill_no] = skill_util.initSkill(skill_no);

			// 技能升級、消除衝突技能
			skill_util.upgSkill(char);
		}

		if (true) {	// 未完成
			// 取得影響角色素質技能
			let talent_skill_data = skill_util.getDataByEffect(char.skill, "char");
			...
			if (char.age == -1 || char.age_t == -1) {
				...
				if (char.age_t == -1) {
					...
					age = skill_util.setStatusByTalent(char, talent_skill_data, age, "age_inside");	// 依角色心理、生理狀態(age_inside)調整角色外表年齡
					age = age * lifespan / race_util.getData(0, "lifespan");	// 計算種族加權(以外表年齡和種族壽命計算出真實年齡)
					age = skill_util.setStatusByTalent(char, talent_skill_data, age, "age_exp");	// 以角色經歷(age_exp)調整角色真實年齡
				...
				if (char.age == -1) {
					...
					age = age * race_util.getData(0, "lifespan") / lifespan;	// 以真實年齡和種族壽命計算並重置外表年齡
					age = skill_util.setStatusByTalent(char, talent_skill_data, age, "age_outside");	// 以角色外貌(age_outside)調整角色外表年齡
			...
			if (char.height == -1 || char.weight == -1 || char.body_b == -1 || char.body_w == -1 || char.body_h == -1) {
				...
				if (char.height != -1) {	// 採用預設角色身高
					height = char.height;
				} else if (char.weight != -1) {	// 以 BMI 和預設角色體重計算基礎身高
					height = char_data.getBaseBodyDataByBMI("height", tmp_gender, char.age, char.weight);
				} else {	// 隨機產生基礎身高
					height = char_data.getBaseBodyData("height", tmp_gender, char.age);
				}
				if (char.weight != -1) {	// 採用預設角色體重
					weight = char.weight;
				} else {	// 以 BMI 和預設角色身高計算基礎體重
					weight = char_data.getBaseBodyDataByBMI("weight", tmp_gender, char.age, height);
				}

				// 依據屬性調整身高體重
				height = skill_util.setStatusByTalent(char, talent_skill_data, height, "height");
				weight = skill_util.setStatusByTalent(char, talent_skill_data, weight, "weight");

				// 依據種族平均值調整身高體重
				let race_height = this.getRaceValue(char, "height");
				let race_weight = this.getRaceValue(char, "weight");
				height = height * race_height / race_util.getData(0, "height");
				weight = weight * race_weight / race_util.getData(0, "weight");
				...
				// 依據屬性調整三圍
				body_b = skill_util.setStatusByTalent(char, talent_skill_data, body_b, "body_b");
				body_b_u = skill_util.setStatusByTalent(char, talent_skill_data, body_b_u, "body_b_u");
				body_w = skill_util.setStatusByTalent(char, talent_skill_data, body_w, "body_w");
				body_h = skill_util.setStatusByTalent(char, talent_skill_data, body_h, "body_h");
	...
	getRaceRandomSkill: function (char, race_index, race_r) {
		let base_race_skill_list = race_util.getData(char.race[race_index], "skill");
		for (let skill_group of base_race_skill_list) {
			...
			if (Object.keys(char.skill) > 0) {
				if (group_f = skill_group["group_f"]) {	// 判斷種族技能前，先移除指定群組技能以避免影響判斷
					for (let key in char.skill) {
						...
						if (skill_util.getData(skill_id, "group_f").split("|").includes(group_f)) {
			...
			if (remove_skill_id != null && skill_util.chkEnableSkill(char, remove_skill_id)) char.skill.push(remove_skill_id);	// 回填移除技能
		}
	},

	// 取得種族屬性
	getRaceValue: function (char, item) {
		let value = race_util.getData(char.race[0], item);
		if (char.race.length > 1) value = (value * char.race_r + race_util.getData(char.race[1], item) * (100 - char.race_r)) / 100;
		return value;
	},
	...
	tranCharData: function (char, item, showHidden) {
		...
		if (item == "gender") {	// 性別
			...
			str = char.age;
			if (Math.pow(Math.abs(char.age - char.age_t), 0.7) > 3) str += " (真實年齡 : " + char.age_t + ")";	// 真實年齡與實際年齡相差太多時才顯示
		} else if (item == "race") {	// 種族
			let race = char.race[0];	// 主要種族
			let race_m = char.race[1];	// 混血種族1

			if (race_m == null) {	// 無混血
				str = race_util.getData(race, "name");
			} else {
				if (char.race_r > 80) {	// 主要種族權重壓過混血種族
					...
					if (race_util.getData(race_sub, "rate") >= 70 || race_util.getData(race_sub_2, "rate") >= 70) {
						let tmp_race_sub = (race_util.getData(race_sub, "rate") >= 70) ? race_sub : race_sub_2;

						str += "半";
						let tmp_race = Math.floor(tmp_race_sub / 10) * 10;	// 顯示混血特徵時僅顯示種族，不顯示亞種
						str += race_util.getData((race_util.contains(tmp_race)) ? tmp_race : tmp_race_sub, "name");
					} else {	// 權重較低(特徵不明顯的種族)不會被視為混血
						str = race_util.getData(race_sub, "name");
					}
				}
			}
			...
			let skill_list = skill_util.getListByTypeFromList(char.skill, ...type_list[item]);

			if (skill_list.length > 0) {
				for (let skill_no of skill_list) {

					str += "[";
					str += skill_util.getData(skill_no, "name");
					if (item == "talent" || item == "skill") {
						let max_lv = skill_util.getData(skill_no, "max_lv");
